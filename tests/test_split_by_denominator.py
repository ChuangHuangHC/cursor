...
from sympy import Add, Integer, simplify, symbols, together  # noqa: E402
...
class TestLongExpression(unittest.TestCase):
    """Many addends must still group correctly (the notebook failure mode)."""

    def test_many_repeated_denominators(self):
        n = 500
        dens = [a, b, c]
        terms = [Integer(i + 1) / dens[i % 3] for i in range(n)]
        expr = Add(*terms)
        result = split_by_denominator(expr, terms=terms)
        self.assertEqual(result["Count"], 3)
        self.assertEqual(result["Denominators"], [a, b, c])
        v = verify_split_by_denominator(expr, terms=terms)
        self.assertTrue(v["SumEqualsOriginal"])

    def test_many_distinct_denominators(self):
        n = 200
        dens = symbols(f"d0:{n}")
        terms = [Integer(1) / d for d in dens]
        expr = Add(*terms)
        result = split_by_denominator(expr, terms=terms)
        self.assertEqual(result["Count"], n)
        self.assertEqual(result["Denominators"], list(dens))
        v = verify_split_by_denominator(expr, terms=terms)
        self.assertTrue(v["SumEqualsOriginal"])
